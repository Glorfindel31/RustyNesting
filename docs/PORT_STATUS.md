...
| Electron file/function | Rust module | Status | Notes / gotchas |
|---|---|---|---|
| `NfpCache` as one shared structure behind a lock | `nesting::cache` | done | Plain `Mutex<HashMap<String, CachedNfp>>` (the honest default per this row's own note — no contention data yet to justify sharding), keyed by Phase 2's unified `cache_key::nfp_cache_key`. `CachedNfp` is an explicit `Outer{outer,children} \| Inner(Vec<Vec<Point>>)` enum instead of the original's `Nfp \| Nfp[]` union disambiguated by a caller-supplied `inner` flag. Real simplification found while porting: the original hand-tracks an `entryCount` field solely to avoid `Object.keys(db).length` being O(n) per insert — Rust's `HashMap::len()` is already O(1), so that bookkeeping isn't needed. Clone-on-read/write (required in JS so a live NFP-tracing mutation of `Point.marked` can't corrupt a cached entry another caller later reads) falls out for free from `Point` being `Copy` — ordinary `Vec::clone()` already produces an independent copy, no custom clone method needed. `MAX_CACHE_ENTRIES = 50000` preserved exactly (bounded memory over an OOM crash on a long run) |
...
| Progress/log events: rayon closures → Tauri/tokio event loop | `nesting::events` | not started | deliberately deferred, not skipped: `nesting::dispatch` has no progress-callback parameter yet (a `Fn` callback invoked from multiple rayon worker threads needs `Sync`, real design work) and there is still no Tauri command layer (Phase 6) to be the actual consumer - building this now would be an interface designed against nothing. Revisit once Phase 6 exists |
