 ...
 */
import { getElement, createSvgElement, setAttributes, addClass, removeClass, } from "../utils/dom-utils.js";
/**
...
export class SheetDialogService {
     ...
     */
    setUpdatePartsCallback(callback) {
     ...
     */
    openDialog() {
        const partsTools = getElement(SELECTORS.PARTS_TOOLS);
        if (partsTools) {
     ...
     */
    clearInputs() {
        const widthInput = getElement(SELECTORS.SHEET_WIDTH_INPUT);
        const heightInput = getElement(SELECTORS.SHEET_HEIGHT_INPUT);
        if (widthInput) {
    ...
    bindEventHandlers() {
        ...
        const addSheetBtn = getElement(SELECTORS.ADD_SHEET_BTN);
        if (addSheetBtn) {
        ...
        const cancelBtn = getElement(SELECTORS.CANCEL_SHEET_BTN);
        if (cancelBtn) {
        ...
        const confirmBtn = getElement(SELECTORS.CONFIRM_SHEET_BTN);
        if (confirmBtn) {
 ...
 * @returns New SheetDialogService instance
 */
export function createSheetDialogService(options) {
 ...
 */
export function initializeSheetDialog(deepNest, config, ractive, resizeCallback) {
...
//# sourceMappingURL=sheet-dialog.js.map