...
export class NestingService {
    ...
    constructor(options) {
        if (options) {
            ...
            this.saveRecoveryFn = options.saveRecoveryFn || null;
            this.updatePartsCallback = options.updatePartsCallback || null;
        }
    }
    /**
     ...
     */
    setDisplayNestFunction(displayNestFn) {
     ...
     */
    setSaveJsonFunction(saveJsonFn) {
     ...
     */
    setSaveRecoveryFunction(saveRecoveryFn) {
        this.saveRecoveryFn = saveRecoveryFn;
    }
    /**
    ...
    createDisplayCallback() {
        return () => {
            ...
            // Snapshot the best-so-far nest to disk every time it improves, independent
            // of what the user has selected/is viewing, so a crash never loses progress
            if (this.saveRecoveryFn && this.deepNest.nests.length > 0) {
                this.saveRecoveryFn(this.deepNest.nests[0]);
            }
    ...
    startNesting(progressCallback) {
        ...
        // Reject upfront if a part's bounding box can't fit any sheet in either
        // orientation - select the offending parts so they're easy to find, instead of
        // letting the placement loop discover it minutes into a slow run.
        const oversized = this.deepNest.getOversizedParts();
        if (oversized.length > 0) {
            this.deepNest.parts.forEach((part) => {
                const isOversized = oversized.includes(part);
                part.selected = isOversized;
            ...
            const names = oversized.map((p) => p.filename || "part").join(", ");
            message(`${oversized.length} part(s) are too large to fit on any sheet in either orientation (selected below): ${names}`, true);
            return false;
        }
 ...
 * @returns New NestingService instance
 */
export function createNestingService(options) {
    return NestingService.create(options);
}
//# sourceMappingURL=nesting.service.js.map